...
function handleErrorsRequest(data, sender, sendResponse) {
	...
	for(var i in data.errors) {
		...
		error.originUrl = sender.tab.url;
...
function uploadError(error){
	if(! error.is404){
		$.get(error.url, function(res){
			error.file = res;
			sendErrorRequest(error);
		})	
	}else{
		sendErrorRequest(error);
	}
...
function sendErrorRequest(data){
	data.timestamp = (new Date()).getTime() / 1000;
	data._eventType = 'browserError';

	var parser = document.createElement('a');
	parser.href = data.originUrl;
	if(parser.host.startsWith('localhost') || parser.protocol === 'file'){
		$.ajax('http://localhost:8000/log', {
